...
aws ec2 run-instances --image-id ${AMI_ID} --instance-type t3.micro --instance-market-options "MarketType=spot,SpotOptions={SpotInstanceType=persistent,InstanceInterruptionBehavior=stop}" --tag-specifications "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=${NAME}}]" "ResourceType=instance,Tags=[{Key=Name,Value=${NAME}}]" --security-group-ids sg-0f2014ce1036b2e77 &>/dev/null
...
aws route53 change-resource-record-sets --hosted-zone-id Z0255309256KJK6OPCR6M --change-batch file:///tmp/record.json &>/dev/null
