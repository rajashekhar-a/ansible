...
CREATE() {
  ...
  if [ $COUNT -eq 0 ]; then
    aws ec2 run-instances --launch-template LaunchTemplateId=lt-045e95080a755d43e  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=$1}]' 'ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]'| jq &>/dev/null
  else
